...
const IncomingCallModal: React.FC<IncomingCallModalProps> = ({
	...
	useEffect(() => {
		if (isVisible) {
			let incomingCallEnabled = true
			let ringtoneVolume = 1
			try {
				incomingCallEnabled =
					localStorage.getItem('app_notif_incoming_call') !== 'false'
				const rawVolume = Number(localStorage.getItem('app_ringtone_volume'))
				if (!Number.isNaN(rawVolume)) {
					ringtoneVolume = Math.min(1, Math.max(0, rawVolume / 100))
				}
			} catch {}
			if (!incomingCallEnabled) return

			if (!ringtoneRef.current) {
				const src = getAttachmentUrl('/static/rington.wav')
				const el = new Audio(src)
				el.loop = true
				el.volume = ringtoneVolume
