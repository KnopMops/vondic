...
- **Authorization Server**: `https://vondic.knopusmedia.ru/oauth`
- **API Base URL**: `https://vondic.knopusmedia.ru/api`
- **Public API**: `https://api.vondic.knopusmedia.ru/api/public/v1`
...
app.get('/login', (req, res) => {
    ...
    const authUrl = `https://vondic.knopusmedia.ru/oauth/authorize?` +
        `client_id=${CLIENT_ID}&` +
...
app.get('/callback', async (req, res) => {
    ...
    try {
        ...
        const userResponse = await axios.get(
            'https://vondic.knopusmedia.ru/oauth/userinfo',
...
if (!isset($_GET['code'])) {
    ...
    $authUrl = 'https://vondic.knopusmedia.ru/oauth/authorize?' . http_build_query([
        'client_id' => $clientId,
...
// Exchange code for token
$ch = curl_init('https://vondic.knopusmedia.ru/oauth/token');
...
// Get user info
$ch = curl_init('https://vondic.knopusmedia.ru/oauth/userinfo');
