...
bot = Bot(
    ...
    base_url="https://vondic.ru",  # API base URL
...
| `modal` | `str \| None` | Opens a Vondic UI modal in the chat client (no bot outbox reply) |

### Bot games (HTML/CSS/JS)

Attach HTML games to a bot. Games are uploaded as ZIP archives with `index.html` at the root. The platform scans them (sync + RabbitMQ worker `game_scans`) and serves them in a sandboxed iframe.

**SDK helpers:**

```python
from botiksdk import InlineKeyboardBuilder, play_games_button, upload_game_button, game_play_button

builder = InlineKeyboardBuilder()
builder.row(play_games_button(), upload_game_button())
await bot.send_message(chat_id, "🎮 Игры", reply_markup=builder.as_markup())
```

| Button / method | Behaviour |
|-----------------|-----------|
| `play_games_button()` | `callback_data=games:list` — бот показывает список игр в **чате с ботом** |
| `game_play_button(id, title)` | `game:play:{id}` — бот отправляет inline-игру (`send_game`) |
| `upload_game_button()` | modal `upload_game` — загрузка ZIP на платформу (проверка) |
| `await bot.list_games()` | `GET /api/public/v1/bots/{id}/games` (Bot token) |
| `await bot.send_game(chat_id, game_id)` | `POST .../send` с полем `game` — iframe в сообщении бота |

Играть пользователь должен в чате с **привязанным ботом**, не в общем UI мессенджера.

**REST API:** `GET/POST /api/v1/bots/{bot_id}/games` (владелец), `GET .../embed` (iframe), публично для бота — `GET /api/public/v1/bots/{bot_id}/games`.
...
# User is redirected to: https://vondic.ru/oauth/authorize?...
...
bot = Bot(
    ...
    base_url=os.getenv("BOT_BASE_URL", "https://vondic.ru")
)
```
