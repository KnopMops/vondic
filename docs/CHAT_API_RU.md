...
**WebSocket (realtime):** `https://webrtc.vondic.ru` (см. `GET /config`)
...
5. [Серверы мессенджера](#серверы-мессенджера)
...
Создайте ключ в **Настройки → Настройки разработчика** на [vondic.ru](https://vondic.ru). Далее все запросы — только с заголовком `X-API-Key`.

Чтобы перевыпустить ключ через API:
...
curl https://api.vondic.ru/api/public/v1/chat/config

# 2. Профиль
curl -H "X-API-Key: YOUR_KEY" \
  https://api.vondic.ru/api/public/v1/chat/me

# 3. Отправить личное сообщение
curl -X POST -H "X-API-Key: YOUR_KEY" -H "Content-Type: application/json" \
  -d '{"content":"Привет!","type":"text"}' \
  https://api.vondic.ru/api/public/v1/chat/dm/USER_ID/messages

# 4. История переписки
curl -H "X-API-Key: YOUR_KEY" \
  "https://api.vondic.ru/api/public/v1/chat/dm/USER_ID/messages?per_page=50"
```
...
{
  "api_version": "1",
  "websocket_url": "https://webrtc.vondic.ru",
  "features": {
...
> **Не путать с сообществами VK.** Публичные страницы с лентой записей (как во ВКонтакте, пункт «Сообщества» в сайдбаре ленты) — это **`/api/v1/social-communities`**, не Chat Embed API.  
> Ниже — только **серверы мессенджера** (несколько text/voice каналов, аналог Discord).
...
Сообщения в канале сообщества отправляются через **`/channels/{channel_id}/messages`** — у каждого канала тот же `id`, что и у зеркального канала в API каналов.

| Метод | Путь | Описание |
|-------|------|----------|
| `POST` | `/communities` | Создать сообщество (сервер) |
| `POST` | `/communities/join` | Войти по `invite_code` |
| `GET` | `/communities` | Мои сообщества |
| `POST` | `/communities/search` | Поиск (`{"query":"название"}`) |
| `GET` | `/communities/{community_id}` | Информация о сообществе |
| `PUT` | `/communities/{community_id}` | Обновить (только владелец) |
| `POST` | `/communities/leave` | Выйти `{"community_id":"..."}` |
| `GET` | `/communities/{community_id}/invite` | Код приглашения |
| `GET` | `/communities/{community_id}/channels` | Список каналов |
| `POST` | `/communities/{community_id}/channels` | Создать канал |
...
{
  "name": "Клан Apex",
  "description": "Официальный сервер клана"
}
```

**Вступление по инвайту** (как invite-ссылка Discord / код VK, но для сервера):
...
**Создание канала внутри сообщества:**
...
**Чтение и отправка сообщений в канале сообщества** — те же эндпоинты, что у обычного канала:

```bash
# История
curl -H "X-API-Key: YOUR_KEY" \
  "https://api.vondic.ru/api/public/v1/chat/channels/CHANNEL_ID/messages?per_page=50"

# Отправить
curl -X POST -H "X-API-Key: YOUR_KEY" -H "Content-Type: application/json" \
  -d '{"content":"Всем привет!","type":"text"}' \
  "https://api.vondic.ru/api/public/v1/chat/channels/CHANNEL_ID/messages"
```

**WebSocket:** в `send_message` передайте `channel_id` канала сообщества (не `community_id`).

**Сравнение сущностей:**

| Сущность | Аналог | Назначение |
|----------|--------|------------|
| DM | Личка VK / DM Discord | Переписка 1:1 |
| Группа | Беседа VK / групповой чат | Общий чат без каналов |
| Канал | Публичный канал / чат | Один чат, инвайт |
| **Сервер** (`/communities` в chat API) | **Discord** | Несколько text/voice каналов в мессенджере |
| **Сообщество VK** | **Группа/страница VK** | Лента записей, сайдбар — `/api/v1/social-communities` |
...
const socket = io('https://webrtc.vondic.ru', {
  auth: { token: apiKey },
...
Для группы: `group_id`. Для канала (в т.ч. канала сообщества): `channel_id`.

### 4. События (основные)

| Событие | Направление | Описание |
|---------|-------------|----------|
| `authenticate` | → сервер | Авторизация (передайте API-ключ в поле `access_token`) |
...
API = "https://api.vondic.ru/api/public/v1/chat"
...
| Сообщества (VK, лента) | `/api/v1/social-communities` | Страницы с постами, сайдбар `/feed/communities` |
