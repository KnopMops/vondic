...
export default function LoginPage() {
  ...
  const { login, loginWithTelegram, isLoading } = useAuth();

  const [loginMethod, setLoginMethod] = useState<"email" | "telegram">("email");
  const [telegramStep, setTelegramStep] = useState<"instruction" | "input">("instruction");
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-900">
      <div className="w-full max-w-md space-y-6 rounded-2xl bg-gray-800 p-8 shadow-xl">
        <div className="flex items-center justify-center gap-3">
          <BrandLogo size={32} />
          <h2 className="text-2xl font-bold text-white">Добро пожаловать в Vondic</h2>
        </div>
        ...
        {loginMethod === "email" ? (
          <form className="mt-8 space-y-6" onSubmit={handleEmailLogin}>
            ...
            <div className="space-y-3">
              ...
              <div className="relative flex items-center justify-center">
                 <span className="bg-gray-800 px-2 text-sm text-gray-500">или</span>
              </div>
          ...
          <div className="mt-8 space-y-6">
            {telegramStep === "instruction" ? (
              <div className="space-y-6 text-center animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="space-y-2">
                  <h3 className="text-lg font-medium text-white">Авторизация через Telegram</h3>
                ...
                <div className="rounded-xl bg-gray-700/50 p-6 ring-1 ring-inset ring-gray-600/50">
                  <p className="mb-3 text-xs text-gray-400 uppercase tracking-wider">Напишите боту</p>
                  <a
              ...
              <form onSubmit={handleTelegramLogin} className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="space-y-2 text-center">
                   <h3 className="text-lg font-medium text-white">Введите ключ</h3>
                   <p className="text-sm text-gray-400">Вставьте ключ в формате <code>ID:SECRET</code></p>
                </div>
            ...
            >
              {telegramStep === "input" ? "Назад к инструкции" : "Вернуться к Email"}
            </button>
          </div>
        )}
