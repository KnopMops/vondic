...
class OAuthClient(Base):
    ...
    client_secret_plain = Column(Text, nullable=True)
            ...
            try:
                ...
                if isinstance(parsed, dict):
                    ...
                    if not isinstance(default_scopes, list):
                        default_scopes = []
                    default_scopes = [str(s).strip() for s in default_scopes if str(s).strip()]
                    return text, logo_url, default_scopes
            except Exception:
                pass
        return raw, "", []

    def set_description_fields(self, text="", logo_url="", default_scopes=None):
        if default_scopes is None:
            default_scopes = []
        clean_scopes = [str(s).strip() for s in default_scopes if str(s).strip()]
        data = {
            "text": text or "",
            "logo_url": logo_url or "",
            "default_scopes": clean_scopes,
        }
        self.description = self._META_PREFIX + json.dumps(data, ensure_ascii=False)
