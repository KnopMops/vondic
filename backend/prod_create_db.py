...
# Не пропускаем bootstrap: он добавит системные таблицы и недостающие столбцы
os.environ.pop("SKIP_DB_BOOTSTRAP", None)

db_path = os.path.join(Config.BASE_DIR, "database.db")

# Создаём приложение и схему моделей
app = create_app()
with app.app_context():
    # Создаём таблицы по моделям SQLAlchemy
    db.create_all()

    # Дополняем схему БД системными таблицами/колонками и видео-моделью
    if db.engine.dialect.name == "sqlite":
        # Users: дополнительные поля, используемые фронтендом
        ...
        ensures = [
            ...
            ("ALTER TABLE users ADD COLUMN is_developer INTEGER DEFAULT 0", "is_developer"),
            ...
            # Видео-поля для UI Vondic Видео
            ...
            ("ALTER TABLE users ADD COLUMN video_subscribers INTEGER DEFAULT 0",
             "video_subscribers"),
            ("ALTER TABLE users ADD COLUMN video_count INTEGER DEFAULT 0", "video_count"),
            ("ALTER TABLE users ADD COLUMN video_likes TEXT", "video_likes"),
            ("ALTER TABLE users ADD COLUMN video_watch_later TEXT", "video_watch_later"),
        ...
        # Gifts catalog с начальными данными
        ...
        # Видео-модель (таблица videos) — для UI Vondic Видео
        ...
        if not vv_cols:
            ...
            """))
            db.session.execute(
                text("CREATE INDEX IF NOT EXISTS idx_video_views_vid ON video_views(video_id)"))
            db.session.execute(text(
